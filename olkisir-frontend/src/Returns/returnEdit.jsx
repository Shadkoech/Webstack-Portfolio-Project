...
export const ReturnEdit = ({ returnId, isOpen, onClose, onUpdateReturn }) => {
  ...
  useEffect(() => {
    if (returnId) {
      axios.get(`http://127.0.0.1:8000/api/returns/${returnId}/`)
        .then(response => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      let response;
      if (returnId) {
        response = await axios.put(`http://127.0.0.1:8000/api/returns/${returnId}/`, returnData);
        setMessage("Return updated successfully");
      } else {
        response = await axios.post("http://127.0.0.1:8000/api/returns", returnData);
        setMessage("Return added successfully");
      }
      console.log("Return action successful:", response.data);

      onUpdateReturn(response.data);
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="flex justify-end">
        ...
        <h2 className="mb-4 text-xl font-bold text-gray-900 dark:text-white">
          {returnId ? "Edit Product" : "Add a new product"}
        </h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
            ...
            <div className="w-full">
              ...
              <input
                ...
                name="return_address"
                id="return_address"
                value={returnData.return_address}
                onChange={handleInputChange}
          ...
          >
            {loading ? (returnId ? "Updating..." : "Adding...") : (returnId ? "Update product" : "Add product")}
          </button>
        </form>
      </div>
    </div>
